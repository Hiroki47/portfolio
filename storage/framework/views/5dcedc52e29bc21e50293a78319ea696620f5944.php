                ...
                <?php $__currentLoopData = $user->following; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $following): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="row align-items-center mt-3">
                        <div class="col-auto">
                            <a href="<?php echo e(route('profile.show', $following->following->id)); ?>">
                                <?php if($following->following->avatar): ?>
                                    <img src="<?php echo e(asset('/storage/avatars/' . $following->following->avatar)); ?>" alt="<?php echo e($following->following->avatar); ?>" class="rounded-circle user-avatar">
                                <?php else: ?>
                        ...
                        <div class="col ps-0 text-truncate">
                            <a href="<?php echo e(route('profile.show', $following->following->id)); ?>" class="text-decoration-none text-dark fw-bold small"><?php echo e($following->following->name); ?></a>
                        </div>
                        <div class="col-auto text-end">
                            <?php if(Auth::user()->id !== $following->following->id): ?>
                                <?php if($following->following->isFollowed()): ?>
                                    <form action="<?php echo e(route('follow.destroy', $following->following->id)); ?>" method="post" class="d-inline">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
                                        <button type="submit" class="border-0 bg-transparent p-0 text-secondary btn-sm">Following</button>
                                    </form>
                                <?php else: ?>
                                    <form action="<?php echo e(route('follow.store', $following->following->id)); ?>" method="post" class="d-inline">
                                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /work/resources/views/users/profile/following.blade.php ENDPATH**/ ?>